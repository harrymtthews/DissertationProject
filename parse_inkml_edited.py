...
def create_json_annotation(root, json_data, group=0, level=0, leaf=False, last_leaf=False, heirarchy=[], points={}, properties=[]):
    ...
    for i, elem in enumerate(list(root.iter())[1:]):
...
def convert_to_mscoco(json_data, ann, author, categories=[], ink_colors=[], category_dir="images/categories"):
    ...
    for obj in json_data:
        ...
        #     ink_colors[label] =  xkcd[len(ink_colors.keys()) % 300]
        # ink_color = tuple(map(int, [x*255 for x in colors.to_rgba(ink_colors[label])]))
    
        markup = marking_pattern.search(label)

        for tr, pts in obj["traces"].items():                
            if markup:
                cv2.polylines(img_ann,[pts],False, (255, 255, 255), thickness=1, lineType=cv2.LINE_AA)
            cv2.polylines(img_org,[pts],False, (255, 255, 255), thickness=1, lineType=cv2.LINE_AA)
        
        cv2.imwrite(ann["original"],img_org)
        cv2.imwrite(ann["labelled"],img_ann)
    
    # with open('idk.txt', 'w') as file:
    #     file.write(anno_objs)
...
if __name__ == "__main__":
    ...
    for idx, input_path in enumerate(sorted(glob.glob(os.path.join(data_dir, "*.inkml")))):
        ...
        #
        #   Edited - Write all trace labels to a text file
        #

        # with open('traces_1.txt', 'w') as file:
        #     for i, t in enumerate(traces['traces']):
        #         file.write(f'Trace {i}\n')
        #         for l, g in t['label']:
        #             file.write(f'({l}, {g}) ')
        #         file.write('\n')
